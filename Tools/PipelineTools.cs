...
namespace AzureDevOpsMcpServer.Tools
{
    ...
    public class PipelineTools
    {
        ...
        public PipelineTools(AzureDevOpsService adoService)
        {
        ...
        public async Task<IEnumerable<Build>> GetBuilds(
            ...
            [Description("A list of build definition IDs to filter by.")] IEnumerable<int>? definitions = null,
        ...
        public async Task<Build> RunPipeline(
            ...
            [Description("The branch to build. If not specified, the default branch is used.")] string? branchName = null)
        {
            var client = await _adoService.GetBuildApiAsync();
            var build = new Build
            {
        ...
        [Description("Update a build stage (cancel, retry, or run). Useful for stage-level control in YAML pipelines.")]
            ...
            [Description("Force retry all jobs in the stage.")] bool forceRetryAllJobs = false)
        {
            var connection = _adoService.Connection;
            var baseUrl = connection.Uri.ToString().TrimEnd('/');
            var url = $"{baseUrl}/{project}/_apis/build/builds/{buildId}/stages/{stageName}?api-version=7.1-preview.1";
