...
namespace AzureDevOpsMcpServer.Tools
{
    ...
    public class CoreTools
    {
        ...
        public CoreTools(AzureDevOpsService adoService)
        {
            _adoService = adoService;
        }

        [McpServerTool(Name = "list_projects")]
        [Description("List all projects in the Azure DevOps organization. Supports filtering by state and name.")]
        public async Task<IEnumerable<TeamProjectReference>> ListProjects(
            [Description("Filter projects by their state. Defaults to 'wellFormed'.")] string stateFilter = "wellFormed",
            ...
            [Description("Filter projects by name. Supports partial matches.")] string? projectNameFilter = null)
        {
            ...
            {
                filteredProjects = projects.Where(p => p.Name.Contains(projectNameFilter, StringComparison.OrdinalIgnoreCase));
            }
        ...
        public async Task<IEnumerable<WebApiTeam>> ListProjectTeams(
            ...
            [Description("If true, only return teams that the authenticated user is a member of.")] bool mine = false,
        ...
        public async Task<string> GetIdentityIds(
            [Description("Search filter for identities (e.g., email or display name).")] string searchFilter)
        {
            var connection = _adoService.Connection;
            var baseUrl = connection.Uri.ToString().TrimEnd('/');
            var url = $"{baseUrl}/_apis/identities?searchFilter=General&filterValue={Uri.EscapeDataString(searchFilter)}&api-version=7.1-preview.1";
