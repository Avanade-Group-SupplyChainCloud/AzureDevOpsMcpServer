...
namespace AzureDevOpsMcpServer.Tools
{
    ...
    public class WorkItemTools
    {
        ...
        public WorkItemTools(AzureDevOpsService adoService)
        {
        ...
        public async Task<WorkItem> GetWorkItem(
            ...
            [Description("A list of fields to include in the response.")] IEnumerable<string>? fields = null,
        ...
        public async Task<WorkItem> CreateWorkItem(
            ...
            [Description("A dictionary of fields and their values for the new work item.")] Dictionary<string, object> fields)
        {
            var client = await _adoService.GetWorkItemTrackingApiAsync();
        ...
        public async Task<WorkItem> UpdateWorkItem(
            ...
            [Description("The project name or ID.")] string project,
            [Description("A dictionary of fields and their values to update.")] Dictionary<string, object> updates)
        {
            var client = await _adoService.GetWorkItemTrackingApiAsync();
            var patchDocument = new JsonPatchDocument();
        ...
        {
            var client = await _adoService.GetWorkItemTrackingApiAsync();
            var wiql = $"SELECT [System.Id], [System.Title], [System.State], [System.AssignedTo] FROM WorkItems WHERE [System.TeamProject] = '{project}' AND ([System.AssignedTo] = @Me OR [System.CreatedBy] = @Me)";
        ...
        public async Task<string> UpdateWorkItemsBatch(
            [Description("JSON array of updates: [{id, op, path, value, format}]. op can be 'Add', 'Replace', or 'Remove'. format is optional ('Html' or 'Markdown').")] string updatesJson)
        {
            ...
            var updates = JsonSerializer.Deserialize<List<BatchUpdateInput>>(updatesJson) ?? new List<BatchUpdateInput>();
            var uniqueIds = updates.Select(u => u.Id).Distinct().ToList();

            var body = uniqueIds.Select(id => new
            {
                method = "PATCH",
                uri = $"/_apis/wit/workitems/{id}?api-version=7.1",
                headers = new Dictionary<string, string> { { "Content-Type", "application/json-patch+json" } },
                body = updates.Where(u => u.Id == id).Select(u => new
                {
                    op = u.Op?.ToLower() ?? "add",
                    path = u.Path,
                    value = u.Value
        ...
        public async Task<WorkItem> LinkWorkItems(
            ...
            [Description("The project name or ID.")] string project,
            [Description("Link type: 'parent', 'child', 'related', 'predecessor', 'successor'.")] string linkType = "related")
        {
            var client = await _adoService.GetWorkItemTrackingApiAsync();
        ...
        public async Task<WorkItem> AddArtifactLink(
            ...
            [Description("The link type name (e.g., 'Build', 'Branch', 'Fixed in Commit').")] string linkType = "ArtifactLink",
            ...
            [Description("The team name or ID.")] string team)
        {
            ...
            return JsonSerializer.Serialize(backlogs, new JsonSerializerOptions { WriteIndented = true });
        }
            ...
            [Description("The backlog ID.")] string backlogId)
        {
            ...
            var connection = _adoService.Connection;
            var baseUrl = connection.Uri.ToString().TrimEnd('/');
            var url = $"{baseUrl}/{project}/{team}/_apis/work/backlogs/{backlogId}/workItems?api-version=7.1";
