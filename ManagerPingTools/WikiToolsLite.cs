...
[McpServerToolType]
public class WikiToolsLite(AzureDevOpsService adoService)
{
    ...
    public async Task<string> ListWikiPages(
        [Description("The wiki identifier or name.")] string wikiIdentifier,
        [Description("Maximum number of pages to return. Defaults to 100.")] int top = 100,
    ...
    )
    {
        ...
        var body = new Dictionary<string, object> { ["top"] = top };
        if (pageViewsForDays > 0)
            body["pageViewsForDays"] = pageViewsForDays;
    ...
    )
    {
        ...
        // SDK overload differs by version; this returns metadata by default.
        // recursionLevel is accepted for schema compatibility but not applied in this SDK call.
        _ = recursionLevel;
